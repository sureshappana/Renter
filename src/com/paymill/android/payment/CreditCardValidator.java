...
public class CreditCardValidator {
	...
	public static boolean validate(Context context, EditText name,
			...
			CardType cardType, EditText amount) {
		boolean valid = true;
		...
		valid = validateCheckAmount(context, amount) && valid;
		return valid;
	}

	private static boolean validateCheckAmount(Context context, EditText amount) {
		if (validateIsEmpty(amount.getText().toString())) {
			amount.setError(context.getString(R.string.pm_emptyMSG));
			return false;
		}
		try {
			...
			amount.setError(context.getString(R.string.pm_wrongAmountFomat));
			return false;
		}
		return true;
	}
		...
		if (date.length() > 4) {
			...
			try {
				ccdate.set(Calendar.YEAR, Integer.parseInt(year)
						+ PaymentFragment.milleniumPrefix * 1000
						+ PaymentFragment.centuryPrefix * 100);
			} catch (NumberFormatException e) {
				dateText.setError(context.getString(R.string.pm_emptyMSG));
				notNumber = true;
			}
